...
:jbake-description: Step-by-step guide to integrating Apache Shiro with Spring Boot applications using the shiro-spring-boot-starter for both web and standalone applications.
:idprefix:
:toc:
...
|shiro.allowAccessByDefault |`false` |Allow access when no filter chain matches. Defaults to false in 3.x.
|===
