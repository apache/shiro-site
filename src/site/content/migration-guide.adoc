...
[NOTE]
====
You *must* either use link:dependency-chain.html[FlowLogix Dependency Chains] or import link:jakarta-ee.html#bom_approach[Shiro BOM] for Jakarta EE 9+ applications to ensure all transitive dependencies use the `jakarta` classifier.
...
[NOTE]
====
You *must* import link:jakarta-ee.html#bom_approach[Shiro BOM] to ensure all transitive dependencies use the `jakarta` classifier.
