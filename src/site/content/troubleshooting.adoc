...
By default, Session and Shiro's Remember Me cookie is HTTP-only and secure. If you're testing in a non-HTTPS environment, you may need to adjust the secure flag for testing purposes.
You can use system property to control this behavior. Set `org.apache.shiro.cookie.secure.disabled` to `true` to disable the secure flag on the session and Remember Me cookies for testing.
